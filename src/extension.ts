...
const availableLicenses: Map<string, LicenseInfo> = new Map<
  string,
  LicenseInfo
>([
  ...
  [
    ...
    {
      displayName: "GPLv2",
      creatorFn: (author, projectName) => new GPLv2(author, projectName)
    }
  ],
  [
    "GPLV3",
    {
      displayName: "GPLv3",
      creatorFn: (author, projectName) => new GPLv3(author, projectName)
    }
  ],
  [
    "LGPLV3",
    {
      displayName: "LGPLv3",
      creatorFn: (author, projectName) => new LGPLv3(author, projectName)
    }
  ],
  [
    "AGPLV3",
    { displayName: "AGPLv3", creatorFn: (author, _) => new AGPLv3(author) }
  ],
  ["MIT", { displayName: "MIT", creatorFn: (author, _) => new MIT(author) }],
  [
    "MPLV2",
    { displayName: "MPLv2", creatorFn: (author, _) => new MPLv2(author) }
  ],
  [
    "CC-BY-3",
    {
      displayName: "CC-BY-3",
      creatorFn: (author, projectName) => new CCBY3(author, projectName)
    }
  ],
  [
    "CC-BY-4",
    {
      displayName: "CC-BY-4",
      creatorFn: (author, projectName) => new CCBY4(author, projectName)
    }
  ],
  [
    "CC-BY-NC-3",
    {
      displayName: "CC-BY-NC-3",
      creatorFn: (author, projectName) => new CCBYNC3(author, projectName)
    }
  ],
  [
    "CC-BY-NC-4",
    {
      displayName: "CC-BY-NC-4",
      creatorFn: (author, projectName) => new CCBYNC4(author, projectName)
    }
  ],
  [
    "CC-BY-NC-ND-3",
    {
      displayName: "CC-BY-NC-ND-3",
      creatorFn: (author, projectName) => new CCBYNCND3(author, projectName)
    }
  ],
  [
    "CC-BY-NC-ND-4",
    {
      displayName: "CC-BY-NC-ND-4",
      creatorFn: (author, projectName) => new CCBYNCND4(author, projectName)
    }
  ],
  [
    "CC-BY-NC-SA-3",
    {
      displayName: "CC-BY-NC-SA-3",
      creatorFn: (author, projectName) => new CCBYNCSA3(author, projectName)
    }
  ],
  [
    "CC-BY-NC-SA-4",
    {
      displayName: "CC-BY-NC-SA-4",
      creatorFn: (author, projectName) => new CCBYNCSA4(author, projectName)
    }
  ],
  [
    "CC-BY-ND-3",
    {
      displayName: "CC-BY-ND-3",
      creatorFn: (author, projectName) => new CCBYND3(author, projectName)
    }
  ],
  [
    "CC-BY-ND-4",
    {
      displayName: "CC-BY-ND-4",
      creatorFn: (author, projectName) => new CCBYND4(author, projectName)
    }
  ],
  [
    "CC-BY-SA-3",
    {
      displayName: "CC-BY-SA-3",
      creatorFn: (author, projectName) => new CCBYSA3(author, projectName)
    }
  ],
  [
    "CC-BY-SA-4",
    {
      displayName: "CC-BY-SA-4",
      creatorFn: (author, projectName) => new CCBYSA4(author, projectName)
    }
  ],
  [
    "CC0-1",
    {
      displayName: "CC0-1",
      creatorFn: (author, projectName) => new CC01(author, projectName)
    }
  ],
  ["UNL", { displayName: "UNL", creatorFn: (author, _) => new WTFPL(author) }],
  [
    "WTFPL",
    { displayName: "WTFPL", creatorFn: (author, _) => new WTFPL(author) }
  ],
...
class Licenser {
  ...
  constructor() {
    ...
    if (licenseType === undefined) {
      vscode.window.showWarningMessage(
        "set your preferred license as 'licenser.license' in configuration. Apache License version 2.0 will be used as default."
      );
    ...
    vscode.commands.registerCommand("extension.createLicenseFile", () => {
      this.create();
    });
    vscode.commands.registerCommand("extension.anyLicenseHeader", () => {
      this.arbitrary();
    });
    vscode.commands.registerCommand("extension.insertLicenseHeader", () => {
      this.insert();
    });
    vscode.commands.registerCommand(
      "extension.insertMultipleLicenseHeaders",
      context => {
        this.insertMultiple(context);
      }
    );
  ...
  private _chooseLicenseType(): Thenable<string> {
    ...
    if (
      isNullOrUndefined(licenseType) ||
      licenseType.toLowerCase() == chooseFromList
    ) {
      return vscode.window.showQuickPick(
        Array.from(availableLicenses.values()).map(info => info.displayName)
      );
    }
  ...
  private _doCreateLicense(root: String, license: License) {
    const uri = vscode.Uri.parse(
      "untitled:" + root + path.sep + defaultLicenseFilename
    );
      ...
      if (!fileContent.includes(header)) {
        ...
        try {
          ...
          vscode.window.showErrorMessage(
            "Error adding license header to files"
          );
        }
      } else {
  ...
  arbitrary() {
    vscode.window
      .showInputBox({
        prompt:
          "Specify the license short name to insert. (see package.json for all the candidates)",
        placeHolder: "AL2"
      })
      .then(shortName => {
  ...
  private _onDidChangeActiveTextEditor() {
    vscode.window.onDidChangeActiveTextEditor(e => {
      let licenserSetting = vscode.workspace.getConfiguration("licenser");
      let autoInsertionDisabled = licenserSetting.get<boolean>(
        "disableAutoHeaderInsertion"
      );
  ...
  private getLicense(typ: string): License {
    ...
    if (licenseKey === "CUSTOM") {
      let customTermsAndConditions = licenserSetting.get<string>(
  ...
  private getLicenseHeader(license: License, langId: string): string {
    let notation = notations[langId]
      ? notations[langId]
      : notations["plaintext"]; // return plaintext's comment when langId is unexpected.

    let licenserSetting = vscode.workspace.getConfiguration("licenser");
    const preferSingleLineStyle = licenserSetting.get<boolean>(
      "useSingleLineStyle",
      true
    );
   ...
   */
  private multiLineCommentHeader(
    license: License,
    start,
    end,
    ornament: string
  ...
  private getAuthor(): string {
    ...
    vscode.window.showWarningMessage(
      "set author name as ’licenser.author’ in configuration. OS username will be used as default."
    );
    switch (os.platform()) {
      case "win32":
        ...
        if (userprofile === undefined) {
          vscode.window.showErrorMessage(
            "Set USERPROFILE in your environment variables."
          );
        }
